...
var url = "http://localhost:27017/todos/";
...
function checkItem(item, url, callback){
  var req = new XMLHttpRequest();
  req.open('PUT', url+item._id, true );
  ...
  req.onreadystatechange = function (){
    if (req.readyState === XMLHttpRequest.DONE){
      ...
      // console.log(JSON.stringify({id: item._id, text: item.text, completed: true}));
      callback();
    }
  }
...
function removeItem(item, url, callback){
  var req = new XMLHttpRequest();
  req.open('DELETE', url+item._id, true);
...
function drawList(TodoList, Listlength){
  ...
  TodoList.forEach(function(item, index){
    ...
    newDiv.setAttribute('data-index-Number', item._id);
    newDiv.setAttribute('data-id', item.completed);
    newDiv.dataset.indexNumber = item._id;
